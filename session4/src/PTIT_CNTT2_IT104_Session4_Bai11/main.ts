...
const converScore = (score: string | number): number => {
...
const updateStudent = (nameStudent: string): void => {
...
const rankingStudent = (student: Student): string => {
    const average = calculateAverage(student);
    if(average < 5) return "Yếu";
    else if(average < 6.5) return "Trung bình";
    else if(average < 8.5) return "Khá";
    else return "Giỏi";
}
